...
config_t
config_parse_from_file (char const file_name[static 1])
{
  ...
  while (0 == feof (cfp))
    {
      ...
      if (NULL == line_buf)
        {
          warn ("failed to read line %zu in file %s, using default", line_num,
                file_name);
          return config_default ();
        }
...
inline usz
config_dim_x (config_t self)
{
  return self.dim_x;
}

inline usz
config_dim_y (config_t self)
{
  return self.dim_y;
}

inline usz
config_dim_z (config_t self)
{
  return self.dim_z;
}

inline usz
config_niter (config_t self)
{
  return self.niter;
}

void
config_print (config_t const *self)
{
  fprintf (stderr,
           "****************************************\n"

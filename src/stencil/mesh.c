...
mesh_t mesh_new(usz dim_x, usz dim_y, usz dim_z, mesh_kind_t kind) {
    ...
    cell_t cells;
    cells.value = (f64***)malloc((dim_x + ghost_size) * sizeof(f64**));
    cells.kind  = (cell_kind_t***)malloc((dim_x + ghost_size) * sizeof(cell_kind_t**));
    if (NULL == cells.value || NULL == cells.kind) {
        error("failed to allocate dimension X of mesh of size %zu bytes", dim_x + ghost_size);
    }
    /*
    NOTE : The i part shall be aligned as we are iterating a lot in it in the jacobi 
    */ 
   for (usz i = 0; i < dim_x + ghost_size; ++i) {
        cells.value[i] = (f64**)malloc((dim_y + ghost_size) * sizeof(f64*));
        cells.kind[i]  = (cell_kind_t**)malloc((dim_y + ghost_size) * sizeof(cell_kind_t*));
        ...
        for (usz j = 0; j < dim_y + ghost_size; ++j) {
            cells.value[i][j] = (f64*)malloc((dim_z + ghost_size) * sizeof(f64));
            cells.kind[i][j]  = (cell_kind_t*)malloc((dim_z + ghost_size) * sizeof(cell_kind_t));

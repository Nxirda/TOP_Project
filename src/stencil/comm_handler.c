...
comm_handler_t comm_handler_new(u32 rank, u32 comm_size, usz dim_x, usz dim_y, usz dim_z) {
    u32 nb_z = gcd(comm_size, (u32)(dim_x * dim_y));
    u32 nb_y = gcd(comm_size / nb_z, (u32)dim_z);
    u32 nb_x = (comm_size / nb_z) / nb_y;

    if (comm_size != nb_x * nb_y * nb_z) {
        error("Splitting does not match MPI communicator size: expected %u, got %u", comm_size, nb_x * nb_y * nb_z);
    }

    u32 rank_z = rank / (comm_size / nb_z);
    u32 rank_y = (rank % (comm_size / nb_z)) / nb_y;
    u32 rank_x = rank % nb_y;

    usz loc_dim_z = (rank_z == nb_z - 1) ? dim_z / nb_z + dim_z % nb_z : dim_z / nb_z;
    usz loc_dim_y = (rank_y == nb_y - 1) ? dim_y / nb_y + dim_y % nb_y : dim_y / nb_y;
    usz loc_dim_x = (rank_x == nb_x - 1) ? dim_x / nb_x + dim_x % nb_x : dim_x / nb_x;

    u32 coord_z = rank_z * (u32)(dim_z / nb_z);
    u32 coord_y = rank_y * (u32)(dim_y / nb_y);
    u32 coord_x = rank_x * (u32)(dim_x / nb_x);

    return (comm_handler_t){
        .nb_x = nb_x, .nb_y = nb_y, .nb_z = nb_z,
        .coord_x = coord_x, .coord_y = coord_y, .coord_z = coord_z,
        .loc_dim_x = loc_dim_x, .loc_dim_y = loc_dim_y, .loc_dim_z = loc_dim_z,
...
void comm_handler_print(const comm_handler_t* self) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    char bt[MAXLEN], bb[MAXLEN], bl[MAXLEN], br[MAXLEN], bf[MAXLEN], bd[MAXLEN];
    fprintf(stderr,
            "****************************************\n"
            "RANK %d:\n"
            "  COORDS:     %u,%u,%u\n"
            ...
            "  %2s  \x1b[1m*\x1b[0m  %2s\n"
            "     %2s  %2s\n",
            rank, self->coord_x, self->coord_y, self->coord_z,
            ...
            for (usz i = x_start; i < x_start + STENCIL_ORDER; ++i) {
                ...
                switch (comm_kind) {
                    case COMM_KIND_SEND_OP:
                        MPI_Send(&mesh->values[index], 1, MPI_DOUBLE, target, 0, MPI_COMM_WORLD);
                        break;
                    case COMM_KIND_RECV_OP:
                        MPI_Recv(&mesh->values[index], 1, MPI_DOUBLE, target, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        break;
                    default:
